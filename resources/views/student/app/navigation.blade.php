<nav class="bg-gradient-to-l from-indigo-200 via-white to-green-100 shadow-lg rounded-b-xl border-b border-indigo-300">
    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex items-center py-3">
        ...
        <x-nav-link :href="route('student.lessons')" :active="request()->routeIs('lessons.*')" class="text-green-700 hover:text-white hover:bg-green-500 transition rounded px-3 py-2 font-semibold">

            الدروس
            </x-nav-link>
        <x-nav-link :href="route('student.exams')" :active="request()->routeIs('exams.*')" class="text-yellow-700 hover:text-white hover:bg-yellow-500 transition rounded px-3 py-2 font-semibold">
            الامتحانات
        </x-nav-link>
        <x-nav-link :href="route('student.notifications')" :active="request()->routeIs('notifications.*')" class="text-pink-700 hover:text-white hover:bg-pink-500 transition rounded px-3 py-2 font-semibold">
            الإشعارات
        </x-nav-link>
...
<x-nav-link :href="route('student.lessons')" :active="request()->routeIs('lessons.*')" class="text-green-700 hover:text-white hover:bg-green-500 transition rounded px-3 py-2 font-semibold">
            ...
            <x-responsive-nav-link :href="route('student.exams')" :active="request()->routeIs('exams.*')" class="block text-yellow-700 hover:text-white hover:bg-yellow-500 transition rounded px-3 py-2 font-semibold">
                الامتحانات
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('student.notifications')" :active="request()->routeIs('notifications.*')" class="block text-pink-700 hover:text-white hover:bg-pink-500 transition rounded px-3 py-2 font-semibold">
                الإشعارات
            </x-responsive-nav-link>
